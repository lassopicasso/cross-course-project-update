  ...
  products.forEach(function (product) {
    ...
    for (let i = 0; i < product.categories.length; i++) {
      if (product.categories[i].slug === "new-women") {
        womenNewArray.push(product);
      }
      if (product.categories[i].slug === "popular-women") {
        womenPopularArray.push(product);
      }
    }

    createHTML(womenNewArray, new_products);
    createHTML(womenPopularArray, popular_products);
  });
}

newProductsButton.onclick = function () {
  document.querySelector(".popularProd").style.display = "none";
  document.querySelector(".newProd h2").innerHTML = `<a href="women.html">Women <i
...
popularProductsButton.onclick = function () {
  ...
  document.querySelector(".popularProd h2").innerHTML = `<a href="women.html">Women <i
